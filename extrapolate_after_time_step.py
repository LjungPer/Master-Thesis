...
first_hits = [10, 20, 30, 40, 50, 60, 70, 80, 90]
...
for first_hit in first_hits:
    ...
    for N in NList:
        ...
        lams[lams > 5] = 5
        lams[lams < 0.01] = 0.01
        for i in range(first_hit, numTimeSteps):
            print 'Extrapolate correction at N = %d, i = %d' % (N, i)
        ...
        errorhits.append(np.sqrt(np.dot((UFine[-1] - VFine[-1] - WFine[-1]), (UFine[-1] - VFine[-1] - WFine[-1]))))
    error.append(errorhits)
...
for i in range(len(first_hits)):
    plt.loglog(NList, error[i], '--s', basex=2, basey=2, label='$n$ = %d' % first_hits[i])
